...
// Function that pushes data for the squares and diamonds question into vertices and colours
void generate_squares_diamonds(MyGeometry *geometry, vector<float>& vertices, vector<float>& colours, int layer) 
{
    float basepos = 0.9f;
    float baseneg = -0.9f;
...
// Function that pushes data for the squares and diamonds question into vertices and colours
void generate_spiral(MyGeometry *geometry, vector<float>& vertices, vector<float>& colours, int layer) 
{
    ...
    {
        float rad = (float)layer*i*2*M_PI/max_radius;
        vertices.push_back((i/max_radius) * (float) cos(rad));
...
// Recursively creates sierpinski triangles based on number of layers and pushes info
// into vertices and colours for rendering
void sierpinski(vector<float>& vertices, vector<float>& colours, Triangle& triangle_prev, int recursions) {

    // Push vertices and colours to buffer if at lowest recursion 
    if (recursions == 1) {
        ...
        // Handle colours if only 1 layer or determine intensity based on distance from 
        // center of window
...
// Function to set starting dimensions for sierpinski triangle
void generate_triangles(MyGeometry *geometry, vector<float>& vertices, vector<float>& colours, int layer) 
{
    Triangle triangle;
...
// state machine used to update the display
void update_display() {
    ...
    switch(current_state.shape) {
        case SQUARES_DIAMONDS:
            generate_squares_diamonds(&global_geo, vertices, colours, current_state.layer);
            break;
        case SPIRAL:
            generate_spiral(&global_geo, vertices, colours, current_state.layer);
            break;
        case TRIANGLES:
            generate_triangles(&global_geo, vertices, colours, current_state.layer);
            break;
    }
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
    ...
    if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS) {
        ...
        if (current_state.layer > 1) {
            current_state.layer--;
            should_update_display = true;
        }
    } else if (key == GLFW_KEY_DOWN && action == GLFW_PRESS) {
        if (current_state.layer < 7) {
            current_state.layer++;
            should_update_display = true;
        }
    } else if (key == GLFW_KEY_Q && action == GLFW_PRESS) {
        current_state.shape = SQUARES_DIAMONDS;
        current_state.layer = 1;
        should_update_display = true;
    } else if (key == GLFW_KEY_W && action == GLFW_PRESS) {
        current_state.shape = SPIRAL;
        current_state.layer = 1;
        should_update_display = true;
    } else if (key == GLFW_KEY_E && action == GLFW_PRESS) {
        current_state.shape = TRIANGLES;
        current_state.layer = 1;
        should_update_display = true;
    }
